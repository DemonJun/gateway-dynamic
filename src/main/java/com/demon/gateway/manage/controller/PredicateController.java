...
public class PredicateController {
    ...
    public Flux<PredicateDefinition> getPredicateList(@PathVariable Integer page, @PathVariable Integer size) {
        log.info("请求方法名：{}，请求参数：page-{},size-{}", "getPredicateList", page, size);
        page -= 1;

        return predicateDefinitionService.getPredicateList(page, size).map(temp -> {
    ...
    @PostMapping
    public Mono<PredicateDefinition> savePredicate(@Validated PredicateDefinition predicateDefinition) {
        log.info("请求方法名：{}，请求参数：{}", "savePredicate", JsonUtils.toJsonString(predicateDefinition));

        return predicateDefinitionService.savePredicate(predicateDefinition).map(temp -> {
    ...
    @PutMapping("/{id}")
    public Mono<Boolean> updatePredicate(@PathVariable String id, @Validated PredicateDefinition predicateDefinition) {
        log.info("请求方法名：{}，请求参数：{}", "updatePredicate", JsonUtils.toJsonString(predicateDefinition));
        return predicateDefinitionService.updatePredicate(id, predicateDefinition).map(temp -> {
    ...
    public Mono<Boolean> deletePredicate(@PathVariable String id) {
        ...
        return predicateDefinitionService.deletePredicate(id).map(temp -> {
