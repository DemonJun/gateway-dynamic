...
public class FilterController {
  ...
  @PutMapping("/{id}")
  public Mono<Boolean> updateFilter(
      @PathVariable String id, @Validated FilterDefinition filterDefinition) {
    log.info("请求方法名：{}，请求参数：{}", "updateFilter", JsonUtils.toJsonString(filterDefinition));
    return filterDefinitionService
        .updateFilter(id, filterDefinition)
        .map(
            temp -> {
