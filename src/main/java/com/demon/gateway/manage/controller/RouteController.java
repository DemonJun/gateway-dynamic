...
public class RouteController {
    ...
    public Flux<RouteDefinition> getRouteList(@PathVariable Integer page, @PathVariable Integer size) {
        log.info("请求方法名：{}，请求参数：page-{},size-{}", "getRouteList", page, size);

        return routeDefinitionService.getRouteList(page, size).map(temp -> {
    ...
    public Mono<RouteDefinition> saveRoute(@Validated RouteDefinition routeDefinition) {
        ...
        return routeDefinitionService.saveRoute(routeDefinition).map(temp -> {
    ...
    @PutMapping("/{id}")
    public Mono<Boolean> updateRoute(@PathVariable String id, @Validated RouteDefinition routeDefinition) {
        log.info("请求方法名：{}，请求参数：{}", "updateRoute", JsonUtils.toJsonString(routeDefinition));
        return routeDefinitionService.updateRoute(id, routeDefinition).map(temp -> {
    ...
    public Mono<Boolean> deleteRoute(@PathVariable String id) {
        ...
        return routeDefinitionService.deleteRoute(id).map(temp -> {
